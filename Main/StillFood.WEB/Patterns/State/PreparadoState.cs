...
{
    public class PreparadoState : State
    {
        public override Common.Enums.eResultadoAccion Accion(Models.NotaPedido pNotaPedido)
        {
            ...
            if (wUsuario != null)
            {
                ...
                string wMensaje = string.Format("<div style='width: 70%;margin: 0 auto;'><div><img src='' /></div><div style='font-size: 35px;color: #A3BD31;'> Hola, {0} </div> <br/> su pedido n° {1} se encuentra listo para su retiro o entrega. <br/> Si usted seleccionó envío a domicilio, su pedido será entregado dentro de las primeras 24 horas habiles. <br/> Puede comunicarse con {2} mediante: {3} para cualquier consulta. <br/> Atentamente, el equipo de StillFood. </div>", wUsuario.NombreApellido, pNotaPedido.Numero, wComercio.Nombre, wComunicacion);
                ...
                {
                    pNotaPedido.IdEstado = Convert.ToInt32(Common.Enums.eEstadosNotasPedido.Praparado);
                    ...
                    wLogService.Agregar(wUsuario.Id, "Pedido En Preparación", "Pedido Preparado", "NotasPedidos");
